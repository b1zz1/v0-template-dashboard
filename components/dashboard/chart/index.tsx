        ...
        <ChartContainer className="md:aspect-[3/1] w-full" config={chartConfig}>
          ...
          >
            ...
            <XAxis
              ...
              tickMargin={12}
              strokeWidth={1.5}
              className="uppercase text-sm fill-foreground/70" // improved contrast from fill-muted-foreground
            />
            <YAxis
              tickLine={false}
              axisLine={false}
              tickMargin={0}
              tickCount={6}
              className="text-sm fill-foreground/70" // improved contrast from fill-muted-foreground
    ...
    <Tabs value={activeTab} onValueChange={handleTabChange} className="max-md:gap-4">
      <div className="flex items-center justify-between mb-4 max-md:contents">
        <TabsList className="max-md:w-full">
          <TabsTrigger value="week" className="text-foreground/80">
            WEEK
          </TabsTrigger>{" "}
          {/* improved contrast */}
          <TabsTrigger value="month" className="text-foreground/80">
            MONTH
          </TabsTrigger>{" "}
          {/* improved contrast */}
          <TabsTrigger value="year" className="text-foreground/80">
            YEAR
          </TabsTrigger>{" "}
          {/* improved contrast */}
        </TabsList>
...
export const ChartLegend = ({
  ...
  return (
    <div className="flex items-center gap-2 uppercase">
      ...
      {/* improved contrast from text-muted-foreground */}
    </div>
  )
}
